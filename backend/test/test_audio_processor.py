...
def mock_transformers():
    with (
        patch("audio_processor.transcriber.WhisperProcessor") as mock_processor,
        patch(
        ...
        processor_instance.batch_decode.return_value = ["This is a test transcription"]
...
def sample_mp3():
    ...
    test_file_path = os.path.join(os.path.dirname(__file__), "object", "sample_test.mp3")
...
def test_audio_transcriber_initialization(mock_transformers):
    ...
    mock_processor.from_pretrained.assert_called_once_with("openai/whisper-tiny")
...
def test_process_audio_object(mock_transformers, sample_mp3):
    ...
    mock_processor.from_pretrained.return_value.batch_decode.assert_called_once()
